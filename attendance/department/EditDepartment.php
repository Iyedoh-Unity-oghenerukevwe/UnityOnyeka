...
if (!empty($data->id) && !empty($data->department_name) && !empty($data->faculty_id)) {
    $id = $data->id;
    ...
    $dup = $conn->prepare("SELECT id FROM `department` WHERE department_name=? AND faculty_id=? AND id!=?");
    $dup->bind_param("sii", $department_name, $faculty_id, $id);
    $dup->execute();
    ...
    $stmt = $conn->prepare("UPDATE `department` SET `department_name` = ?, `faculty_id` = ? WHERE `id` = ?");
